...
// === API cho Whitelist (Admin) ===

export const fetchWhitelist = async () => {
  try {
    const response = await axiosInstance.get("/api/whitelist");
    // Đảm bảo luôn trả về một mảng, kể cả khi response.data không tồn tại
    return response.data || [];
  } catch (error) {
    console.error("Lỗi khi tải whitelist:", error);
    // Khi có lỗi, trả về một mảng rỗng để giao diện không bị crash
    return [];
  }
};

export const addWhitelistEmail = async (data) => {
  // data = { name, email }
  try {
    const response = await axiosInstance.post("/api/whitelist", data);
...
export const deleteWhitelistEmail = async (entryId) => {
  try {
    const response = await axiosInstance.delete(`/api/whitelist/${entryId}`);
    if (response.data.error) throw new Error(response.data.error);
    return response.data;
  } catch (error) {
    // ... xử lý lỗi
