...
class AgentResponseHistoryStep(models.Model):
    ...
    def confirm_and_review(self):
        ...
        elif self.agent_response_history_id.copilot_agent_id.agent_code == "009":
            output_response = eval(self.output_response)
            if output_response:
                statement_line_ids = []
                ...
                for statement in output_response:
                    reconcile_possible = False
                    consider_to_check = False
                    statement_id = statement.get('statement_id')
                    statement_date = statement.get('statement_date')
                    statement_reference = statement.get('statement_reference')
                    statement_amount = statement.get('statement_amount', 0.00)
                    matched_transactions = statement.get('matched_transactions')
                    statement_line_rec = statement_line.browse(int(statement_id))
                    commands = list()
                    if statement_line_rec and not statement_line_rec.is_reconciled and statement_line_rec.move_id:
                        move_id_rec = statement_line_rec.move_id
                        journal_id_rec = move_id_rec.journal_id
                        suspense_account = journal_id_rec.suspense_account_id
                        default_account = journal_id_rec.default_account_id

                        if matched_transactions:
                            for transaction in matched_transactions:
                                transaction_id = transaction.get('transaction_id')
                                transaction_partner = transaction.get('transaction_partner')
                                ...
                                if transaction_id != 'New' and transaction_reference !='To Check':
                                    account_move_line_rec = account_move_line.browse(int(transaction_id))
                                    suspense_lines_to_rmv = move_id_rec.line_ids.filtered(lambda l: l.account_id.id == suspense_account.id)[0]

                                    new_move_line = {
                                        'name': account_move_line_rec.name,
                                        'account_id': account_move_line_rec.account_id.id,
                                        'debit': account_move_line_rec.credit,
                                        'credit': account_move_line_rec.debit,
                                        'partner_id': account_move_line_rec.partner_id.id or statement_line_rec.partner_id.id,
                                    }

                                    commands.append((2, suspense_lines_to_rmv.id))
                                    commands.append((0, 0, new_move_line))
                                    reconcile_possible = True

                                elif transaction_id == 'New':
                                    match = re.search(r"\((\d+)\)", transaction_partner)
                                    if match:
                                        rule_id = int(match.group(1))
                                        if rule_id:
                                            reconcile_model_rec = reconcile_model.browse(rule_id)
                                            if reconcile_model_rec and reconcile_model_rec.line_ids:
                                                for rule_line in reconcile_model_rec.line_ids:
                                                    bank_line_to_keep = move_id_rec.line_ids.filtered(lambda l: l.account_id.id == default_account.id)[0]
                                                    suspense_lines_to_rmv = move_id_rec.line_ids.filtered(lambda l: l.account_id.id == suspense_account.id)[0]
                                                    statement_amount = float(statement_amount)
                                                    if rule_line.amount_type in ['fixed']:
                                                        if statement_amount < 0 and float(suspense_lines_to_rmv.debit) != float(rule_line.amount_string):
                                                            consider_to_check = True
                                                            break
                                                        elif statement_amount > 0 and float(suspense_lines_to_rmv.credit) != float(rule_line.amount_string):
                                                            consider_to_check = True
                                                            break
                                                    elif rule_line.amount_type in ['percentage','percentage_st_line']:
                                                        if statement_amount < 0 and float(suspense_lines_to_rmv.debit) != float(bank_line_to_keep.credit)*(float(rule_line.amount_string)/100):
                                                            consider_to_check = True
                                                            break
                                                        elif statement_amount > 0 and float(suspense_lines_to_rmv.credit) != float(bank_line_to_keep.debit)*(float(rule_line.amount_string)/100):
                                                            consider_to_check = True
                                                            break

                                                    new_move_line = {
                                                        'name': reconcile_model_rec.display_name,
                                                        'account_id': rule_line.account_id.id,
                                                        'debit': bank_line_to_keep.credit,
                                                        'credit': bank_line_to_keep.debit,
                                                    }
                                ...
                                elif transaction_reference == 'To Check':
                                    statement_line_rec.to_check = True
                        if consider_to_check:
                            statement_line_rec.to_check = True
                        if reconcile_possible:
                            move_id_rec.button_draft()
                            move_id_rec.write({
                                'line_ids': list(commands)
                            })
                            move_id_rec.action_post()
                            statement_line_rec.is_reconciled = True

                    statement_line_ids.append(statement_id)
                self.agent_response_history_id.copilot_agent_id.status = 'done'
                self.agent_response_history_id.action_state = 'done'
                return {
                    'type': 'ir.actions.act_window',
