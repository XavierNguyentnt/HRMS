...
from odoo.addons.odoo_ai_agent.models.call_back import search_internet_and_interpret_results_with_llm, DecisionMakingAgent, ForecastProductSale, CalculateLeadTime, BankReconciliationMatching
...
class AgentResponseHistory(models.Model):
    ...
    def create_response_line(self, answer):
        ...
        if python_code:
            try:
                ...
                full_response = ""
                if final_step and not agent_code in ["004","008","002", "006", "007"]:
    ...
    def store_formated_data(self, output_dictionary, agent_code):
        ...
        if agent_code == "009":
            bank_reconciliation_data = output_dictionary.get('bank_reconciliation_overview')
            criteria_dic = dict()
            for mdata in bank_reconciliation_data:
                bank_statement_id = mdata.get('statement_id')
                if bank_statement_id not in criteria_dic:
                    criteria_dic[bank_statement_id] = {
                        'statement_id': mdata.get('statement_id') or ' ',
                        'statement_date': mdata.get('statement_date') or ' ',
                        'statement_reference': mdata.get('statement_reference') or ' ',
                        'statement_amount': mdata.get('statement_amount') or ' ',
                        'matched_transactions': []
                    }
                child_dict = {
                    'transaction_id': mdata.get('transaction_id') or ' ',
                    'transaction_partner': mdata.get('transaction_partner') or ' ',
                    'transaction_date': mdata.get('transaction_date') or ' ',
                    'transaction_reference': mdata.get('transaction_reference') or ' ',
                    'transaction_amount': mdata.get('transaction_amount') or ' ',
                }
                criteria_dic[bank_statement_id]['matched_transactions'].append(child_dict)
            formatted_result = list(criteria_dic.values())

            return [formatted_result, output_dictionary]
